...
cast send $REWARD_SYSTEM_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
...
cast send $TOKEN_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
"approve(address,uint256)" $REWARD_SYSTEM_ADDRESS 1000000

cast send $REWARD_SYSTEM_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
"depositRewards(string,string,uint256)" $CLUSTER_ID $ROLLUP_ID 1000000

cast send $REWARD_SYSTEM_ADDRESS "distributeRewards(string,string,bytes32,uint48,bytes,bytes,uint256)" $CLUSTER_ID $ROLLUP_ID 0xa6990e50b43e0c4966db704a0a6390be1c258b89afdd4ae4a02ac141771f5120 1702483200 0x0001 0x0001 100 --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY
...
cast call $REWARDS_CORE_ADDRESS --rpc-url $LIVENESS_RPC_URL \
...
cast send $TOKEN_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $SECONDARY_PRIVATE_KEY \
"approve(address,uint256)" $REWARD_SYSTEM_ADDRESS 1000000
...
cast send $REWARD_SYSTEM_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $SECONDARY_PRIVATE_KEY \
...
cast send 0xa63bA08402aF74196569F3442b3a44946Cf149eA --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
...
cast send $REWARD_SYSTEM_ADDRESS "distributeRewards(string,string,bytes32,uint48,bytes,bytes,uint256)" $CLUSTER_ID $ROLLUP_ID 0x0c3af26cf4bd766f3a2a565a9ae5991c3cf1530c75214d6876726447860c759d 1702483200 0x0001 0x0001 100 --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY
...
cast send $REWARD_TOKEN_ADDRESS "approve(address,uint256)" $DEFAULT_OPERATOR_REWARDS 1000000 --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY
...
cast call $DEFAULT_OPERATOR_REWARDS --rpc-url $LIVENESS_RPC_URL \
"balance(address,address)(uint256)" $NETWORK_ADDRESS $TOKEN_ADDRESS

# Query claimed amount
cast call $OPERATOR_REWARDS_ADDRESS --rpc-url $LIVENESS_RPC_URL \
...
cast send $VALIDATION_SERVICE_MANAGER_CONTRACT_ADDRESS "distributeRewards(string,string,bytes32,uint48,bytes,bytes,uint256)" $CLUSTER_ID $ROLLUP_ID 0xa6990e50b43e0c4966db704a0a6390be1c258b89afdd4ae4a02ac141771f5120 1702483200 0x0001 0x0001 100 --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY
...
#         bytes32[] calldata proof
  ...
  cast send $DEFAULT_OPERATOR_REWARDS --private-key $OPERATING_PRIVATE_KEY --rpc-url $LIVENESS_RPC_URL \
...
cast send $SIMULATION --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY "emitPoolCreated(string,string,address)" $CLUSTER_ID $ROLLUP_ID $REWARD_TOKEN_ADDRESS

cast send $SIMULATION --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
...
cast send $VALIDATION_SERVICE_MANAGER_CONTRACT_ADDRESS "createNewTask(string,string,uint256,bytes32,bytes32[],uint256[])" $CLUSTER_ID $ROLLUP_ID 12 0x287b58b93ed6c17ace087bb87f611bf21102c0602b0956736b6e523fb41c328d [] [] --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_OPERATOR_PRIVATE_KEY
...
cast send <contract-address> "createNewTask(string,string,uint256,bytes32,address[],bytes32[],uint256[])" \
 ...
 cast send $VALIDATION_SERVICE_MANAGER_CONTRACT_ADDRESS "createNewTask(string,string,uint256,bytes32,address[],bytes32[],uint256[])" $CLUSTER_ID $ROLLUP_ID 12 0x287b58b93ed6c17ace087bb87f611bf21102c0602b0956736b6e523fb41c328d [] [] [] --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_OPERATOR_PRIVATE_KEY


cast send $DEFAULT_STAKER_REWARDS "claimRewards(address,address,bytes)" $DEFAULT_ACCOUNT_ADDRESS $DEFAULT_TOKEN_ADDRESS $(cast --from-utf8 $(python3 -c "import eth_abi; print('0x' + eth_abi.encode(['address', 'uint256', 'bytes[]'], ['$NETWORK_ADDRESS', 115792089237316195423570985008687907853269984665640564039457584007913129639935, []]).hex())")) --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_ACCOUNT_PRIVATE_KEY
