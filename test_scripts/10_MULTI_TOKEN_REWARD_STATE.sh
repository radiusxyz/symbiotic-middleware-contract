...
# # Default Token Setup
cast send $DEFAULT_TOKEN_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $TOKEN_CONTRACT_OWNER_PRIVATE_KEY \
"transfer(address,uint256)" $DEFAULT_ACCOUNT_ADDRESS 10000

cast call $DEFAULT_TOKEN_ADDRESS --rpc-url $LIVENESS_RPC_URL \
"balanceOf(address)(uint256)" $DEFAULT_ACCOUNT_ADDRESS

cast send $DEFAULT_TOKEN_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_ACCOUNT_PRIVATE_KEY \
...
cast send $DEFAULT_COLLATERAL_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_ACCOUNT_PRIVATE_KEY \
"deposit(address recipient, uint256 amount)(uint256)" $DEFAULT_ACCOUNT_ADDRESS 10000

cast call $DEFAULT_COLLATERAL_ADDRESS --rpc-url $LIVENESS_RPC_URL \
"balanceOf(address)(uint256)" $DEFAULT_ACCOUNT_ADDRESS

cast send $DEFAULT_COLLATERAL_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_ACCOUNT_PRIVATE_KEY \
"approve(address spender, uint256 value)(bool)" $DEFAULT_VAULT_ADDRESS 10000

cast send $DEFAULT_VAULT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_ACCOUNT_PRIVATE_KEY \
"deposit(address onBehalfOf, uint256 amount)(uint256 depositedAmount, uint256 mintedShares)" $DEFAULT_ACCOUNT_ADDRESS 10000

cast call $DEFAULT_VAULT_ADDRESS --rpc-url $LIVENESS_RPC_URL \
...
cast send $OPERATOR_REGISTRY_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_OPERATOR_PRIVATE_KEY_SECONDARY \
"registerOperator()"
...
cast send $OPERATOR_NETWORK_OPT_IN_SERVICE_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_OPERATOR_PRIVATE_KEY_SECONDARY \
"optIn(address network)" $NETWORK_ADDRESS
...
cast send $OPERATOR_VAULT_OPT_IN_SERVICE_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $DEFAULT_OPERATOR_PRIVATE_KEY_SECONDARY \
"optIn(address vault)" $DEFAULT_VAULT_ADDRESS
...
# Default Delegator
cast send $DEFAULT_DELEGATOR_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
"setMaxNetworkLimit(uint96 identifier, uint256 amount)" 0 10000

cast send $DEFAULT_DELEGATOR_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $VAULT_OWNER_PRIVATE_KEY \
