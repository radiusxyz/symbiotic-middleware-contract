...
DEFAULT_STAKER_REWARDS=${DEFAULT_STAKER_REWARDS:-"YOUR_STAKER_REWARDS_CONTRACT_ADDRESS"}
DEFAULT_ACCOUNT_ADDRESS=${DEFAULT_ACCOUNT_ADDRESS:-"YOUR_ACCOUNT_ADDRESS"}
...
DEFAULT_ACCOUNT_PRIVATE_KEY=${DEFAULT_ACCOUNT_PRIVATE_KEY:-"YOUR_PRIVATE_KEY"}

echo "Using the following configuration:"
echo "Staker Rewards Contract: $DEFAULT_STAKER_REWARDS"
echo "Account Address: $DEFAULT_ACCOUNT_ADDRESS"
...
BALANCE_BEFORE=$(cast call $DEFAULT_TOKEN_ADDRESS "balanceOf(address)(uint256)" $DEFAULT_ACCOUNT_ADDRESS --rpc-url $RPC_URL)
...
if [ $? -eq 0 ]; then
    ...
    BALANCE_AFTER=$(cast call $DEFAULT_TOKEN_ADDRESS "balanceOf(address)(uint256)" $DEFAULT_ACCOUNT_ADDRESS --rpc-url $RPC_URL)
