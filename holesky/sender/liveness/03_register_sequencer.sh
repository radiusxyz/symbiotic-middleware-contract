...
sequencers=$(cast call $LIVENESS_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL \
...
if echo "$sequencers" | grep -q "$OPERATING_ADDRESS"; then
    ...
    result=$(cast send $LIVENESS_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $OPERATING_PRIVATE_KEY \
