...
clusters=$(cast call $LIVENESS_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL \
"getClustersByOwner(address owner)(string[])" $NETWORK_ADDRESS)

if echo "$clusters" | grep -q "$CLUSTER_ID"; then
    echo "Cluster already exists"
else
    result=$(cast send $LIVENESS_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $NETWORK_PRIVATE_KEY \
...
rollupInfo=$(cast call $LIVENESS_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL \
