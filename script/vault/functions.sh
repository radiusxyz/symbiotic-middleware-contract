...
cast send 0x010F5B294C54b8A5A53B10694C31CDB50Ab4C857 --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY --value 1000000000000000000
cast send 0xD19c3705A7e5143c8a80Fdb2d7e90ED537521764 --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY --value 1000000000000000000

cast chain-id --rpc-url $LIVENESS_RPC_URL
...
cast send $TEST_ERC20_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
"approve(address,uint256)" $COLLATERAL_CONTRACT_ADDRESS 1000

# Collateral - deposit - ERC20을 collateral에 예치
cast send $COLLATERAL_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
"deposit(address recipient, uint256 amount) (uint256)" $OPERATOR_ADDRESS 100

# Collateral - balanceOf - operator의 잔액 확인
cast call $COLLATERAL_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL \
...
cast send $VAULT_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
...
cast send $DELEGATOR_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
"setMaxNetworkLimit(uint96 identifier, uint256 amount)" 0 100 

# Subnetwork의 최대 네트워크 제한 확인
cast call $DELEGATOR_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
...
cast call $DELEGATOR_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL "operatorNetworkShares(bytes32 subnetwork, address operator)(uint256)" $SUBNETWORK $OPERATOR_ADDRESS

# Subnetwork에 총 share 확인
cast call $DELEGATOR_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL "totalOperatorNetworkShares(bytes32 subnetwork)(uint256)" $SUBNETWORK
...
cast call $OPERATOR_VAULT_OPT_IN_SERVICE_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
"isOptedIn(address who, address where)(bool)" $OPERATOR_ADDRESS $VAULT_ADDRESS

cast call $NETWORK_OPTIN_SERVICE_CONTRACT_ADDRESS --rpc-url $LIVENESS_RPC_URL --private-key $PRIVATE_KEY \
...
cast call $VAULT_CONTRACT_ADDRESS "depositWhitelist()(bool)" --rpc-url $LIVENESS_RPC_URL
