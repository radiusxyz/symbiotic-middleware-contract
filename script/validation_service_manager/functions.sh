...
cast call --rpc-url $RPC_URL $VALIDATION_SERVICE_MANAGER_CONTRACT_ADDRESS "getOperatorStake(address operator, uint48 epoch)(uint256)" $OPERATOR_ADDRESS 112
cast call --rpc-url $RPC_URL $VALIDATION_SERVICE_MANAGER_CONTRACT_ADDRESS "getTotalStake(uint48 epoch)(uint256)" 112
